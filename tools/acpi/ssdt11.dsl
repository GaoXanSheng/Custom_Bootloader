 ...
 *
 * Disassembly of f:/ECtools/isal/ssdt11.dat
 *
...
DefinitionBlock ("", "SSDT", 2, "INSYDE", "EDK2    ", 0x00000001)
{
    ...
    External (_SB_.PCI0.GPP2._ADR, IntObj)
    External (_SB_.PCI0.GPP2.PWEN, MethodObj)    // 0 Arguments
    ...
    Scope (\_SB.PCI0.GPP2)
    {
        ...
        {
            M460 ("PLA-ASL-_SB.PCI0.GPP2.PWEN\n", Zero, Zero, Zero, Zero, Zero, Zero)
            ...
            If (CondRefOf (\_SB.PCI0.GPP2._ADR))
            {
                ...
                ADDR = \_SB.PCI0.GPP2._ADR /* External reference */
            }
            ...
            Local3 = M009 (Local1)
            Local6 = CLKR /* \_SB_.PCI0.GPP2.PWEN.CLKR */
        ...
        {
            M460 ("PLA-ASL-_SB.PCI0.GPP2.GPCG Bus = 0x%X Dev = 0x%X Fun = 0x%X\n", Arg0, Arg1, Arg2, Zero, Zero, Zero)
            Local0 = Zero
            ...
            M460 ("PLA-ASL-_SB.PCI0.GPP2.GPCG Bus = 0x%X Dev = 0x%X Fun = 0x%X PCIe Cap Offset = 0x%X\n", Arg0, Arg1, Arg2, Local0, Zero, Zero)
            Return (Local0)
        }

        Method (RPCF, 0, NotSerialized)
        {
            M460 ("PLA-ASL-_SB.PCI0.GPP2.RPCF NBRI=0x%X\n", NBRI, Zero, Zero, Zero, Zero, Zero)
            If (CondRefOf (\_SB.PCI0.GPP2._ADR))
            {
                Local1 = GPCG (NBRI, Zero, Zero)
                M460 ("PLA-ASL-_SB.PCI0.GPP2.RPCF PXDC = 0x%X PXLC = 0x%X PXD2 = 0x%X NBAR = 0x%X NCMD = 0x%X\n", PXDC, PXLC, PXD2, NBAR, NCMD, Zero)
        ...
        {
            M460 ("PLA-ASL-_SB.PCI0.GPP2.SPCF\n", Zero, Zero, Zero, Zero, Zero, Zero)
            If (CondRefOf (\_SB.PCI0.GPP2._ADR))
            {
                Name (ADDR, Buffer (0x04)
                {
                ...
                CreateByteField (ADDR, 0x02, PDEV)
                ADDR = \_SB.PCI0.GPP2._ADR /* External reference */
                M460 ("PLA-ASL-_SB.PCI0.GPP2.SPCF Dev = 0x%X Func = 0x%X\n", PDEV, PFUN, Zero, Zero, Zero, Zero)
                Local0 = M019 (Zero, PDEV, PFUN, 0x18)
                ...
                M460 ("PLA-ASL-_SB.PCI0.GPP2.RPCF NBRI = 0x%X NCMD = 0x%X NBAR = 0x%X \n", NBRI, NCMD, NBAR, Zero, Zero, Zero)
                M460 ("PLA-ASL-_SB.PCI0.GPP2.RPCF PXDC = 0x%X PXLC = 0x%X PXD2 = 0x%X \n", PXDC, PXLC, PXD2, Zero, Zero, Zero)
                LCR1 = M019 (Zero, PDEV, PFUN, 0x68)
                M460 ("PLA-ASL-_SB.PCI0.GPP2.PWDI.LCR1 : %x\n", LCR1, Zero, Zero, Zero, Zero, Zero)
                LCR2 = M019 (Zero, PDEV, PFUN, 0x88)
                M460 ("PLA-ASL-_SB.PCI0.GPP2.PWDI.LCR2 : %x\n", LCR2, Zero, Zero, Zero, Zero, Zero)
            }
        }
        ...
        PowerResource (P0NV, 0x00, 0x0000)
        {
            ...
            {
                M000 (0x30AA)
                M460 ("PLA-ASL-\\_SB.PCI0.GPP2.P0NV._STA\n", Zero, Zero, Zero, Zero, Zero, Zero)
                Return (D0NV) /* \_SB_.PCI0.GPP2.P0NV.D0NV */
            }

            Method (_ON, 0, NotSerialized)  // _ON_: Power On
            {
                M000 (0x30D0)
                M460 ("PLA-ASL-\\_SB.PCI0.GPP2.P0NV._ON\n", Zero, Zero, Zero, Zero, Zero, Zero)
                Local0 = DerefOf (DerefOf (D3PK [Zero]) [One])
                Local1 = DerefOf (DerefOf (D3PK [0x06]) [One])
                Local2 = 0x02
                M460 ("PLA-ASL-\\_SB.PCI0.GPP2.P0NV._ON D3Cold = 0x%X WD3C First = 0x%X SI3V = 0x%X\n", Local0, WD3C, SI3V, Zero, Zero, Zero)
                If ((Local0 == One))
                {
                    PWEN ()
                }
                ...
                If (((WD3C == One) || (SI3V == One)))
                {
                    ...
                    Else
                    {
                        While (((Local1 != Zero) && (Local1 <= 0x03)))
                        {
                            ...
                            M460 ("PLA-ASL-\\_SB.PCI0.GPP2.P0NV._ON SMI Index = 0x%X Data = 0x%X\n", Local2, Local3, Zero, Zero, Zero, Zero)
                            ASMO = Local3
                            Local2++
                            Local1--
                            Sleep (0x0A)
            ...
            {
                M000 (0x30D3)
                M460 ("PLA-ASL-\\_SB.PCI0.GPP2.P0NV._OFF\n", Zero, Zero, Zero, Zero, Zero, Zero)
                SPCF ()
        ...
        Device (NVME)
        {
            ...
            {
                M000 (0x3050)
                M460 ("PLA-ASL-\\_SB.PCI0.GPP2.NVME._PS0\n", Zero, Zero, Zero, Zero, Zero, Zero)
            }

            Method (_PS3, 0, NotSerialized)  // _PS3: Power State 3
            {
                M000 (0x3053)
                M460 ("PLA-ASL-\\_SB.PCI0.GPP2.NVME._PS3\n", Zero, Zero, Zero, Zero, Zero, Zero)
            }
